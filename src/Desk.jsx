...
export default function Desk() {
  ...
  // Candidate volunteers come from the people already registered (deduped names)
  // plus Anish. Fallback if none.
...
function Walkin({ user, ward, flash, onDone }) {
  ...
  const [f, setF] = useState({ name: '', phone: '', ward: ward, paid: '', method: '', auto: false });
  ...
  async function submit(e) {
    ...
    try {
      ...
      setF({ name: '', phone: '', ward: ward, paid: '', method: '', auto: false });
      setOpen(false);
      onDone();
    } catch (err) {
      flash('Error: ' + err.message);
